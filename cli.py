#
# cli.py
# 全体の役割：iwa_rnaseq_counter の各実行モードに対する統一入口。単一アッセイ実行、バッチ実行、行列統合、および GUI バックグラウンド実行をサブコマンドとして提供。
# 共通機能：標準出力・ファイルへのロギング設定の一元化、内部ソースコード (src) のパス解決、引数バリデーションを担当。
# ---
# 主要なサブコマンド
# 1. run-assay (単一アッセイ実行):
# 単一の AssaySpec を受け取り、定量計算を実行。アッセイレベルの MatrixSpec と実行記録を出力。
# 主に特定サンプルの単体実行や、開発時の検証用途を想定。
# 
# 2. run-batch (バッチ実行):
# サンプルシートを読み込み、複数のアッセイを連続実行。出力ディレクトリを検体（Specimen）単位で整理。
# リファレンス（Salmon Index 等）を一括適用し、大規模なデータセットの定量計算を自動化。
# 
# 3. build-analysis-matrix (解析行列の統合):
# アッセイ単位の複数の MatrixSpec を統合し、解析（Analysis）単位の統合行列を構築。
# サンプル属性（メタデータ）の付与と、下流の Reporter モジュールに適合する標準 Spec への変換を行う。
# 
# 4. run-gui-backend (GUIバックエンド実行):
# GUI（app.py）が作成した実行構成 (run_config.json) を受け取り、非同期ジョブとしてパイプラインを起動。
# GUI からバックグラウンド処理への「橋渡し」となるエントリーポイント。
# 
