...
def build_matrix_spec_from_gui_result(
    ...
    """
    GUIによる生成した集計カウントマトリックスのMatrixSpecを構築
    """
    matrix_abs_path = (run_dir / matrix_rel_path).resolve()
    
    # GUIには明示的なアッセイIDがないため、サンプルIDから合成
    ...
    metadata = {
        ...
        # [v0.6.0 C-09]
        # MatrixSpec.metadata は比較的 backend 非依存に見えるが、
        # backend / quantifier 情報をどこまでここへ載せるか未整理。
        # v0.6.0 では run_summary 由来の backend 情報を
        # ここで勝手に再解釈しない方が安全。
    }
...
def build_execution_run_spec_from_gui_result(
    ...
    """
    GUI実行全体に対するExecutionRunSpecを構築
    """
    ...
    parameters = {
        ...
        # [v0.6.0 C-05 / C-09]
        # GUI adapter が quantifier 名を固定値 "salmon" で埋めている。
        # 本来 adapter 層は backend 実装詳細を直書きせず、
        # run_summary / backend metadata / normalized execution context から受け取るべき。
        "quantifier": "salmon"
    }
    ...
    return ExecutionRunSpec(
        ...
        # [v0.6.0 C-09]
        # execution_backend="local-gui" は「実行形態」と「backend」を少し混ぜている。
        # GUI 実行であることと quantifier/backend 種別は本来別軸。
        # v0.6.0 では execution_backend の意味定義も見直し候補。
...
def write_gui_supporting_inputs(
    ...
    """
    GUI仕様生成のメインエントリーポイント。
    GUIの結果を標準モデルにマッピングする仕様を生成し、保存する。
    """
    ...
    # [v0.6.0 C-03 / C-09]
    # この関数は GUI 実行結果を Spec へ橋渡しする adapter の本体。
    # v0.6.0 では run_summary の backend 由来情報と
    # feature_annotation 契約の入力境界をここで整理するのが自然。
