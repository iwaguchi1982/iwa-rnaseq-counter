...
def run_counter_pipeline(assay_spec: AssaySpec, outdir: Path, threads: int = 4) -> tuple[MatrixSpec, ExecutionRunSpec]:
    ...
    all_paths = r1_paths + r2_paths if layout_final == "single-end" else []
    ...
    if tx2gene:
        ...
        matrix_spec = MatrixSpec(
            ...
            source_subject_ids=[], 
            metadata={
                "producer_app": "iwa-rnaseq-counter",
                ...
                "feature_annotation_available": False,
            }
        )
    else:
        raise NotImplementedError("Transcript-level un-aggregated output is not currently handled without tx2gene.")

    # v0.1: emit spec-based producer outputs first.
    # legacy dataset-manifest style outputs remain handled by the existing app flow.
    finished_at = datetime.now(timezone.utc).astimezone().isoformat()
    
    run_spec = ExecutionRunSpec(
        schema_name="ExecutionRunSpec",
        schema_version="0.1.0",
        run_id=f"RUN_COUNTER_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
