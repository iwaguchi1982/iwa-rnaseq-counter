...
### 7.5 使い勝手は追求するが、run artifact の法は守る
利用者には保存先 root を選べる柔軟性を持たせる。  
ただし、run directory の内部構造は固定し、Job History や次段アプリ連携が壊れないようにする。
...
### v0.4.2 に向けた現在の整理
- GUI からの実行を job として扱い、バックグラウンド実行できる形が見えてきた
- Job History で実行中 / 完了 / 失敗の状態を見られるようにしている
- 一覧を読む親ディレクトリは `Runs Root` で指定可能にする方針
- ただし、固定するのは run の内部構造であり、root だけを選べるようにする
- 原則は「root は選べる / run の中身は固定」
